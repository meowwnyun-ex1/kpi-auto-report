...
router.get('/dept/entries', async (req, res) => {
  try {
    ...
    if (department_id) {
      ...
      request.input('department_id', sql.Int, Number(department_id));
    }
...
router.get('/dept/by-department/:departmentId', async (req, res) => {
  try {
    ...
    const request = pool
      .request()
      .input('department_id', sql.Int, Number(departmentId))
...
router.put('/dept/update', async (req, res) => {
  try {
    ...
    const result_db = await pool
      ...
      .input('result', sql.NVarChar, result || null)
      .input('accu_target', sql.NVarChar, accu_target || null)
      .input('accu_result', sql.NVarChar, accu_result || null).query(`
...
router.get('/dept/dashboard', async (req, res) => {
  try {
    ...
    const deptSummaryResult = await pool.request().input('year', sql.Int, Number(year)).query(`
        SELECT 
