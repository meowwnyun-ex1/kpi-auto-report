...
import { TripModel } from '../models/ApplicationModels';
...
router.get('/', async (req, res) => {
  try {
    ...
    const trips = await TripModel.getAll(pool, active === 'true');
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const trip = await TripModel.getById(pool, parseInt(id));
...
router.post('/', upload.single('image'), async (req, res) => {
  try {
    ...
    const trip = await TripModel.create(pool, {
...
router.put('/:id', upload.single('image'), async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      const existingTrip = await TripModel.getById(
        pool,
    ...
    const updateData: Parameters<typeof TripModel.update>[2] = {
      title: req.body.title,
      link_url: req.body.link_url || undefined,
      is_active: req.body.is_active === 'true' || req.body.is_active === true,
      sort_order: parseInt(req.body.sort_order, 10) || 0,
    ...
    const trip = await TripModel.update(pool, parseInt(Array.isArray(id) ? id[0] : id), updateData);
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await TripModel.delete(pool, parseInt(id));
