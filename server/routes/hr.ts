...
router.get('/summary/by-subcategory', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool
      ...
      .input('month', sql.NVarChar, (month as string) || 'Apr').query(`
        SELECT 
          sc.id,
          sc.name_en,
          sc.[key],
 ...
 */
router.get('/dept/summary', async (req: Request, res: Response) => {
  try {
    const pool = await getKpiDb();
    ...
    const departmentSummary = metrics.reduce((acc: any, m: any) => {
    ...
    const summary = {
      total_metrics: metrics.length,
      departments: Object.values(departmentSummary).map((dept: any) => ({
        ...dept,
 ...
 */
router.get('/dept/departments', async (req: Request, res: Response) => {
  try {
    const pool = await getKpiDb();
 ...
 */
router.get('/dept/sub-categories', async (req: Request, res: Response) => {
  try {
    const pool = await getKpiDb();
 ...
 */
router.get('/dept/metrics', async (req: Request, res: Response) => {
  try {
    ...
    const request = pool.request();
    if (department) {
      request.input('department', sql.NVarChar, department as string);
    }
    if (subCategory) {
      request.input('subCategory', sql.NVarChar, subCategory as string);
    }
 ...
 */
router.get('/dept/metrics/:metricId/entries', async (req: Request, res: Response) => {
  try {
    const metricId = Array.isArray(req.params.metricId)
      ? req.params.metricId[0]
      : req.params.metricId;
 ...
 */
router.get('/dept/trend', async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/dept/by-month', async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/dept/years', async (req: Request, res: Response) => {
  try {
    const pool = await getKpiDb();
 ...
 */
router.put('/dept/entries/:id', async (req: Request, res: Response) => {
  try {
    const entryId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const {
      target,
 ...
 */
router.get('/dept/dashboard', async (req: Request, res: Response) => {
  try {
    const pool = await getKpiDb();
