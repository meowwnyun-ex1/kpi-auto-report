...
import { appIdParamSchema } from '../utils/validation-schemas';
...
router.get('/', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const stats = result.recordset[0] as Stats;
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const db = await getAppStoreDb();
      const { appId } = req.params as unknown as { appId: number };
      ...
      if (result.recordset.length === 0) {
        return next(new NotFoundError('Application', appId));
      }
 ...
 * @access Public
 */
router.post('/page-views', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { page } = req.body;
...
router.get('/categories', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const distribution = result.recordset.map((row: Record<string, unknown>) => ({
      name: row.name,
      icon: row.icon,
      count: row.count,
 ...
 * @access Public
 */
router.get('/app-stats-over-time', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const db = await getAppStoreDb();
    ...
    const stats = result.recordset.map((row: Record<string, unknown>) => ({
      month: row.month,
      year: row.year,
      count: row.count,
    }));
...
router.get('/system-health', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const activeResources =
      (health.active_categories || 0) + (health.active_banners || 0) + (health.active_trips || 0);
 ...
 * @access Public
 */
router.get('/engagement-metrics', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const db = await getAppStoreDb();
    ...
    const metrics = result.recordset.map((row: Record<string, unknown>) => ({
      name: (row.day_name as string)?.substring(0, 3) || 'N/A',
      views: row.views || 0,
      apps: row.apps || 0,
    }));
 ...
 * @access Public
 */
router.get('/growth-forecast', async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const db = await getAppStoreDb();
    ...
    const historicalData: ForecastData[] = result.recordset.map((row: Record<string, unknown>) => ({
      month: (row.month_name as string)?.substring(0, 3) || 'N/A',
      actual: (row.actual as number) || 0,
      forecast: 0,
    }));

    // Calculate average growth rate from non-zero months
    const nonZeroMonths = historicalData.filter((d: ForecastData) => d.actual > 0);
    let growthRate = 1.1; // Default 10% growth

    if (nonZeroMonths.length >= 2) {
      const recentValues = nonZeroMonths.slice(-3).map((d: ForecastData) => d.actual);
      const avgRecent =
        recentValues.reduce((a: number, b: number) => a + b, 0) / recentValues.length;
      const olderValues = nonZeroMonths.slice(0, -3).map((d: ForecastData) => d.actual);
      const avgOlder =
        olderValues.length > 0
          ? olderValues.reduce((a: number, b: number) => a + b, 0) / olderValues.length
          : avgRecent;
