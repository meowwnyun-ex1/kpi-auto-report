      ...
      `);
        ...
        if (safetyId) {
            const worksiteSub = await pool
                .request()
                .input('category_id', safetyId)
                .query(`SELECT id FROM kpi_sub_categories WHERE category_id = @category_id AND [key] = 'worksite'`);
            const worksiteId = worksiteSub.recordset[0]?.id;
            const trafficSub = await pool
                .request()
                .input('category_id', safetyId)
                .query(`SELECT id FROM kpi_sub_categories WHERE category_id = @category_id AND [key] = 'traffic'`);
