...
exports.CategoryModel = exports.TripModel = exports.BannerModel = exports.ApplicationModel = void 0;
...
class ApplicationModel {
    ...
    `;
        ...
        if (filters.category_id) {
            ...
            request.input('category_id', mssql_1.default.Int, filters.category_id);
        }
        if (filters.status) {
        ...
        const sortBy = filters.sortBy && allowedSortBy.includes(filters.sortBy) ? filters.sortBy : 'created_at';
    ...
    static async getById(db, id) {
        const result = await db.request().input('id', mssql_1.default.Int, id).query(`
        SELECT 
          a.id,
          a.name,
          a.url,
    ...
    static async create(db, data) {
        const result = await db
            ...
            .input('url', mssql_1.default.NVarChar(1000), data.url)
            .input('status', mssql_1.default.NVarChar(50), data.status || 'pending')
            .input('is_active', mssql_1.default.Bit, data.is_active !== false)
            .input('category_id', mssql_1.default.Int, data.category_id || null)
            .input('image_thumbnail', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_thumbnail || null)
            .input('image_small', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_small || null)
            .input('image_path', mssql_1.default.NVarChar(500), data.image_path || null)
            .input('image_metadata', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_metadata || null).query(`
    ...
    static async update(db, id, data) {
        const setClauses = [];
        const request = db.request().input('id', mssql_1.default.Int, id);
        if (data.name !== undefined) {
            setClauses.push('name = @name');
            request.input('name', mssql_1.default.NVarChar(255), data.name);
        }
        if (data.url !== undefined) {
            setClauses.push('url = @url');
            request.input('url', mssql_1.default.NVarChar(1000), data.url);
        }
        if (data.category_id !== undefined) {
            setClauses.push('category_id = @category_id');
            request.input('category_id', mssql_1.default.Int, data.category_id);
        }
        if (data.image_thumbnail !== undefined) {
            setClauses.push('image_thumbnail = @image_thumbnail');
            request.input('image_thumbnail', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_thumbnail);
        }
        if (data.image_small !== undefined) {
            setClauses.push('image_small = @image_small');
            request.input('image_small', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_small);
        }
        if (data.image_path !== undefined) {
            setClauses.push('image_path = @image_path');
            request.input('image_path', mssql_1.default.NVarChar(500), data.image_path);
        }
        if (data.image_metadata !== undefined) {
            setClauses.push('image_metadata = @image_metadata');
            request.input('image_metadata', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_metadata);
        }
        if (data.status !== undefined) {
            setClauses.push('status = @status');
            request.input('status', mssql_1.default.NVarChar(50), data.status);
        }
        if (data.is_active !== undefined) {
            setClauses.push('is_active = @is_active');
            request.input('is_active', mssql_1.default.Bit, data.is_active);
        }
    ...
    static async incrementViewCount(db, id) {
        await db
            .request()
            .input('id', mssql_1.default.Int, id)
    ...
    static async updateViewCount(db, id) {
        return this.incrementViewCount(db, id);
    }
    // Get applications by category
    static async getByCategory(categoryId) {
        const db = await (0, database_1.getAppStoreDb)();
        const result = await db.request().input('category_id', mssql_1.default.Int, categoryId).query(`
        SELECT 
          a.id,
          a.name,
          a.url,
...
class BannerModel {
    static async getAll(pool, activeOnly = true) {
        let query = `
    ...
    static async create(pool, data) {
        const result = await pool
            .request()
            .input('title', mssql_1.default.NVarChar(200), data.title)
            .input('link_url', mssql_1.default.NVarChar(2000), data.link_url || null)
            .input('image_thumbnail', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_thumbnail || null)
            .input('image_small', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_small || null)
            .input('image_path', mssql_1.default.NVarChar(500), data.image_path || null)
            .input('image_metadata', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_metadata || null)
            .input('is_active', mssql_1.default.Bit, data.is_active !== false)
            .input('sort_order', mssql_1.default.Int, data.sort_order || 0).query(`
    ...
    static async update(pool, id, data) {
        ...
        if (data.title !== undefined) {
            ...
            request.input('title', mssql_1.default.NVarChar(200), data.title);
        }
        if (data.link_url !== undefined) {
            setClauses.push('link_url = @link_url');
            request.input('link_url', mssql_1.default.NVarChar(2000), data.link_url);
        }
        if (data.image_thumbnail !== undefined) {
            setClauses.push('image_thumbnail = @image_thumbnail');
            request.input('image_thumbnail', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_thumbnail);
        ...
        if (data.sort_order !== undefined) {
            ...
            request.input('sort_order', mssql_1.default.Int, data.sort_order);
        }
        setClauses.push('updated_at = GETDATE()');
        await request.query(`
      UPDATE banners
    ...
    static async delete(pool, id) {
        await pool.request().input('id', mssql_1.default.Int, id).query('DELETE FROM banners WHERE id = @id');
    }
}
...
class TripModel {
    ...
    static async delete(pool, id) {
        await pool.request().input('id', mssql_1.default.Int, id).query('DELETE FROM trips WHERE id = @id');
    }
}
...
class CategoryModel {
    ...
    static async create(pool, data) {
        const result = await pool
            .request()
            .input('name', mssql_1.default.NVarChar(200), data.name)
            .input('icon', mssql_1.default.NVarChar(mssql_1.default.MAX), data.icon || null)
            .input('is_active', mssql_1.default.Bit, data.is_active !== false)
            .input('image_thumbnail', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_thumbnail || null)
            .input('image_small', mssql_1.default.NVarChar(mssql_1.default.MAX), data.image_small || null)
            .input('image_path', mssql_1.default.NVarChar(500), data.image_path || null)
    ...
    static async update(pool, id, data) {
        ...
        if (data.name !== undefined) {
            setClauses.push('name = @name');
            request.input('name', mssql_1.default.NVarChar(200), data.name);
        }
        if (data.icon !== undefined) {
            ...
            request.input('icon', mssql_1.default.NVarChar(mssql_1.default.MAX), iconValue);
        }
        if (data.is_active !== undefined) {
            setClauses.push('is_active = @is_active');
            request.input('is_active', mssql_1.default.Bit, data.is_active);
    ...
    static async delete(pool, id) {
        await pool.request().input('id', mssql_1.default.Int, id).query('DELETE FROM categories WHERE id = @id');
    }
}
