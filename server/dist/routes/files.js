...
router.get('/', auth_1.requireAuth, (0, auth_1.requireRole)('admin'), async (_req, res) => {
    ...
    `);
        appsResult.recordset.forEach((app) => {
      ...
      WHERE image_thumbnail IS NOT NULL OR image_small IS NOT NULL OR image_path IS NOT NULL
    `);
        bannersResult.recordset.forEach((banner) => {
      ...
      WHERE image_thumbnail IS NOT NULL OR image_small IS NOT NULL OR image_path IS NOT NULL
    `);
        tripsResult.recordset.forEach((trip) => {
...
router.get('/stats/storage', auth_1.requireAuth, (0, auth_1.requireRole)('admin'), async (_req, res) => {
    try {
        const pool = await (0, database_1.getAppStoreDb)();
    ...
    `);
        ...
        const diskStats = ImageStorage_1.ImageStorage.getStorageStats();
...
router.post('/cleanup', auth_1.requireAuth, (0, auth_1.requireRole)('admin'), async (_req, res) => {
    try {
        await ImageStorage_1.ImageStorage.cleanupAll();
        res.json({
            success: true,
        ...
        logger_1.logger.error('Failed to cleanup files:', error);
        res.status(500).json({
            success: false,
