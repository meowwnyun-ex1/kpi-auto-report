...
router.get('/quality/summary', async (req, res) => {
    ...
    `);
        ...
        const summary = {
            ...
            claim_metrics: metrics.filter((m) => m.sub_category_key === 'claim').length,
            loss_metrics: metrics.filter((m) => m.sub_category_key === 'loss').length,
 ...
 */
router.get('/quality/metrics/:metricId/entries', async (req, res) => {
    try {
 ...
 */
router.get('/quality/products', async (req, res) => {
    try {
 ...
 */
router.get('/quality/comparison', async (req, res) => {
    try {
        ...
        const result = await pool
            ...
            .input('subCategory', mssql_1.default.NVarChar, subCategory || 'claim').query(`
        WITH MonthlyData AS (
          SELECT 
            m.id as metric_id,
            m.no,
