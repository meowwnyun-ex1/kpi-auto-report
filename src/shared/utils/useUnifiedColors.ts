...
import { useSystemStandards, useSystemValidation } from './useSystemStandards';
...
export function useUnifiedColors() {
  ...
  const { validateComponent } = useSystemValidation();
  ...
  const getCategoryColor = (categoryKey: string): TextPriority => {
    return (
      UNIFIED_COLOR_MAPPINGS.categories[
        categoryKey as keyof typeof UNIFIED_COLOR_MAPPINGS.categories
      ] || 'neutral'
    );
  };
  ...
  const validateColorUsage = (componentName: string, colors: string[]) => {
    ...
    colors.forEach((color) => {
      if (hardcodedColors.includes(color)) {
        issues.push(
          `Component "${componentName}" uses hardcoded color "${color}" - should use priority system`
        );
      }
    });
...
export function useColorConsistency() {
  ...
  const getStandardizedColorClass = (
    percentage: number,
      ...
      success:
        type === 'text'
          ...
          : type === 'bg'
            ...
            : 'border-emerald-200',
      positive:
        type === 'text' ? 'text-green-700' : type === 'bg' ? 'bg-green-50' : 'border-green-200',
