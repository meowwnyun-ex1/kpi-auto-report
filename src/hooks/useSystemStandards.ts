...
import {
  ...
  getPageTheme,
  getComponentMapping,
} from '@/constants/system-standards';
import { TextPriority } from '@/constants/text-priority-colors';
import { PriorityLevel } from '@/constants/priority-colors';
...
export function useAnimationStyles(
  type: keyof typeof SYSTEM_STANDARDS.animations
) {
  const styles = useMemo(() => {
...
export function useSystemValidation() {
  const validateComponent = (componentName: string, props: any) => {
    ...
    // ตรวจสอบว่าใช้ Priority components หรือไม่
    if (!componentName.includes('Priority')) {
  ...
  const config = useMemo(() => {
    return {
      ...
      kpi: {
        ...
        targetThreshold: 100,
        diffThreshold: 5,
        categories: ['safety', 'quality', 'delivery', 'compliance', 'hr', 'attractive', 'environment', 'cost'],
      },
